...
int zmachine_load_story(ZMachine *vm, const char *path)
{
    ...
    /* V6 and V7 define these offsets. V7 uses them for packed addresses.
     * We intentionally reject V6, but parse V7 so the address helpers are
     * correct from the beginning. */
    vm->routine_offset = (version == 7) ? read_be16(buf + 0x28) : 0;
    vm->string_offset = (version == 7) ? read_be16(buf + 0x2A) : 0;

    /* Header extension is available in V5 and later. */
...
int zmachine_run(ZMachine *vm)
{
    static const char message[] =
        "Z-machine core loaded successfully. Opcode execution is not implemented yet.\n";
    ...
    /*
     * Starter milestone only.
     *
     * The next implementation phase replaces this stub with the instruction
     * decoder/executor. The contract is to run until the VM requests line
     * input, halts, or errors. Only textual output is appended to vm->output.
     * Screen positioning, graphics, mouse, sound, fonts and other rich
     * presentation features are intentionally outside the runtime's scope.
     */
    zmachine_output_clear(vm);
    zmachine_output_append(vm, message, sizeof(message) - 1U);
