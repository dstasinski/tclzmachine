 ...
 *
 * Tcl supplies an entire command line at once.  This module converts that
 * line into the exact text-buffer representation expected by the active
 * Z-machine version, lowercases it for dictionary matching, and optionally
 * fills the story's parse buffer using the dictionary declared in the story
 * header.  It intentionally contains no terminal or IRC behavior.
 */
...
/*
 * Look up one already-tokenized word in the story dictionary.
 *
 * The dictionary header supplies a separator list, entry width, and signed
 * entry count.  A negative count indicates an unsorted table, but linear
 * search is valid for both sorted and unsorted dictionaries and keeps this
...
/* Return nonzero when c is one of the story-defined token separators. */
 ...
 * separator is emitted as a one-character token.  Each standard parse entry
 * is four bytes: dictionary address, token length, and byte offset into the
 * story text buffer.  text_offset is 1 in V1-V4 and 2 in V5+ because the
 * physical text-buffer layouts differ between those version families.
 */
                    ...
                    const char *line, size_t len, uint8_t text_offset)
{
    ...
    /* A zero parse-buffer operand means "do not tokenize". */
    if (parse_buffer == 0U)
        return TCL_OK;
 ...
 * the actual character count in byte 1, and begin text at byte 2.  The input
 * is truncated to the story-advertised capacity before it is written.
 */
